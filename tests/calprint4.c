...
void print_events(node_t *events, time_t *start_date, time_t *end_date) {
    ...
    for(; curr != NULL; curr = curr->next){
        ...
        printf("end_date");
        ptime(end_date);
        printf("difftime curr start: %f\n",difftime(*curr_date, *start_date));
        printf("difftime end curr: %f\n", difftime(*end_date, *curr_date));
...
time_t *get_date(char *time_s){
    ...
    time_t *date = convert_time(new_time);
    free(new_time);
    return date;
}
...
event_t *get_event(char *line, event_t *temp_event){
    ...
    if(strncmp(line, "SUMMARY:", 8) == 0){
        ...
        strncat(temp_event->summary, "\0", 2);
        //printf("summar: %s\n", temp_event->summary);
        //temp_event->summary = realloc(temp_event->summary, sizeof(char) * strlen(temp_event->summary));
    }
    return temp_event;
}
